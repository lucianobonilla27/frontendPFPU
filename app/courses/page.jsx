...
import { PlusCircle, Pencil, Trash2, AlertCircle, Users, Loader2, CheckCircle, X, Info } from "lucide-react"
...
const CourseManagement = () => {
  ...
  const fetchCourses = useCallback(async () => {
    try {
      ...
      showNotification("error", "No se pudieron cargar los cursos. Por favor, intenta de nuevo más tarde.")
    } finally {
      setLoading(false)
    }
  }, [])
  ...
  const createCourse = async (courseData) => {
    try {
      ...
      const gradeName = getGradeName(courseData.id_anio)
      showNotification("success", `El curso ${gradeName} "${courseData.division}" ha sido creado exitosamente.`)
      ...
      showNotification("error", "No se pudo crear el curso. Por favor, intenta de nuevo.")
    } finally {
  ...
  const updateCourse = async (courseData) => {
    try {
      ...
      // Mostrar notificación de éxito
      const gradeName = getGradeName(courseData.id_anio)
      showNotification("success", `El curso ${gradeName} "${courseData.division}" ha sido actualizado exitosamente.`)
      ...
      showNotification("error", "No se pudo actualizar el curso. Por favor, intenta de nuevo.")
    } finally {
      setIsSubmitting(false)
    }
  }
  ...
  const deleteCourse = async () => {
    ...
    try {
      ...
      // Mostrar notificación de éxito
      const gradeName = getGradeName(selectedCourseForDelete.id_anio)
      showNotification(
        "success",
        `El curso ${gradeName} "${selectedCourseForDelete.division}" ha sido eliminado exitosamente.`,
      )
      ...
      showNotification("error", "No se pudo eliminar el curso. Por favor, intenta de nuevo.")
    } finally {
      setIsSubmitting(false)
    }
  }
